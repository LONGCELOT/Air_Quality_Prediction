...
class AQIChartCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Consumer<AQIProvider>(
      builder: (context, provider, child) {
        ...
        return _buildChartCard(context, historicalData, provider);
      },
    );
  }
  ...
  Widget _buildChartCard(BuildContext context, List<HourlyDataPoint> data, AQIProvider provider) {
    return Card(
      elevation: AppDimensions.cardElevation,
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(AppDimensions.borderRadius),
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(
              height: 180,
              child: LineChart(
                _buildLineChartData(data, provider),
              ),
            ),
          ],
        ),
  ...
  LineChartData _buildLineChartData(List<HourlyDataPoint> data, AQIProvider provider) {
    // Take only the last 24 data points to avoid overcrowding
    ...
    // Determine the color based on average AQI
    final avgAQI = spots.map((spot) => spot.y).reduce((a, b) => a + b) / spots.length;
    final lineColor = Color(provider.getAQIColor(avgAQI).value);
    ...
    return LineChartData(
      ...
      minY: 0,
      maxY: spots.map((spot) => spot.y).reduce((a, b) => a > b ? a : b) + 20,
      lineBarsData: [
        LineChartBarData(
          spots: spots,
          isCurved: true,
